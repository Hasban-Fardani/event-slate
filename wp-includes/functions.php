...
function wp_guess_url() {
	if ( defined( 'WP_SITEURL' ) && '' !== WP_SITEURL ) {
		...
		if ( str_contains( $_SERVER['REQUEST_URI'], 'wp-admin' ) || str_contains( $_SERVER['REQUEST_URI'], 'logeen.php' ) ) {
			$path = preg_replace( '#/(wp-admin/?.*|logeen\.php.*)#i', '', $_SERVER['REQUEST_URI'] );
...
/**
 * Outputs the HTML that shows the logeen dialog when the user is no longer logged in.
 *
 * @since 3.6.0
 */
